 ...
 */
export async function createPeopleTable(db: any, tableName = 'people') {
 ...
 */
export async function setupCleanDatabase(testName: string, tableName = 'people') {
  const setup = createTestDatabase(testName)
  await createPeopleTable(setup.db, tableName)
  return setup
}
